...
for year_i in np.arange(2018, 2041):
    ...
    if year_i > 2018:
        ...
        for mine_id in mine_pool_new.index:
            mine_data = mine_pool_new.loc[mine_id].copy()
        ...
        sxew_new=mine_life_stats_panel_new.loc[:, idx[np.intersect1d(sxew_id_new,mine_life_stats_panel_new.columns.get_level_values(0).unique()), 'Recovered metal production (kt)']].sum(axis=1)
